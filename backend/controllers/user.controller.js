...
const registerUser = asyncHandler( async(req, res) => {
    ...
    const coverImage = await uploadOnCloudinary(coverImageLocalPath)
        .catch((error) => {
            ...
            // Cover image is optional, so we'll continue
            return null;
        });

    // console.log(avatar);null
    if (!avatar) throw new ApiError(400, "Avatar upload failed. Please check file format and try again.")
...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    console.log(`Channel search results: ${channel?.length ? 'Found' : 'Not found'}`);
...
// Get user profile with their content (videos, tweets)
const getUserProfile = asyncHandler(async (req, res) => {
    ...
    // Prepare user data with content
    const userData = {
        ...user._doc,
        videos,
        tweets
