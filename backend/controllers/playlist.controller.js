...
const getUserPlaylistsByUsername = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Now fetch playlists by user ID
        const playlists = await Playlist.aggregate([
            {
                $match: {
                    owner: user._id
        ...
        console.log(`Found ${playlists.length} playlists for username: ${username}`);
        ...
        console.error(`Error in getUserPlaylistsByUsername: ${error}`);
